...
export default function BulkCsvImportDialog({
  onClose,
}: {
  onClose: () => void;
}) {
  ...
  const [mappedRows, setMappedRows] = useState<BulkMappedRow[]>([]);
  ...
  function goToAccounts() {
    ...
    const accounts = (existingAccounts ?? []) as Account[];
    const existingLower = new Map(
      accounts.map((a) => [a.name.toLowerCase(), a]),
    );

    const mappings: AccountMapping[] = uniqueNames.map((name) => {
      const match = existingLower.get(name.toLowerCase());
      return {
        name,
        type: match?.type ?? "depository",
  ...
  function goToCategories() {
    const hasCategoryColumn = columnRoles.includes("category");
    const uniqueCats = [
      ...new Set(
        mappedRows
          .map((r) => r.category)
    ...
    if (!hasCategoryColumn || uniqueCats.length === 0) {
      goToOwners();
      return;
    }

    const existingNames = (userCategories ?? []).map((c) => c.name);
    const matches = matchCategories(uniqueCats, existingNames);

    const mappings: CategoryMapping[] = matches.map((m) => ({
      csvCategory: m.csvCategory,
      mappedTo: m.suggestion ?? m.csvCategory,
  ...
  function applyCategoryMappings() {
    const catMap = new Map(
      categoryMappings.map((m) => [m.csvCategory, m.mappedTo]),
    );
    setMappedRows((prev) =>
      prev.map((row) => ({
        ...row,
        category: row.category ? (catMap.get(row.category) ?? row.category) : row.category,
      })),
    );
  }
  ...
  function goToOwners() {
    ...
    const mappings: OwnerMapping[] = uniqueOwners.map((name) => {
      const match = household.members.find(
        (m) => m.name.toLowerCase() === name.toLowerCase(),
      );
      return { name, memberId: match?.user_id ?? null };
    });
    setOwnerMappings(mappings);
    setStep("owners");
  }
  ...
  async function startImport() {
    ...
    const newAccounts: BulkImportPayload["accounts"] = accountMappings
      .filter((m) => m.existingId === null)
      .map((m) => ({ name: m.name, type: m.type }));

    const newCategories = categoryMappings
      .filter((m) => m.isNew)
      .map((m) => m.mappedTo);

    const payload: BulkImportPayload = {
      accounts: newAccounts,
      transactions: mappedRows.map((r) => ({
        date: r.date,
        amount: r.amount,
      ...
      new_categories: newCategories.length > 0 ? newCategories : undefined,
    };
  ...
  const accountsPerGroup = Object.entries(
    mappedRows.reduce(
      (acc, r) => {
        const key = r.account_name || "(no account)";
        acc[key] = (acc[key] || 0) + 1;
        return acc;
      },
  ...
  const newAccountCount = accountMappings.filter(
    (m) => m.existingId === null,
  ).length;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-3xl rounded-2xl border border-border bg-card shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Import across multiple accounts from a single CSV
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {step !== "result" && (
          <StepIndicator
            current={step}
            hasHousehold={!!household && household.members.length > 1}
            hasCategories={categoryMappings.length > 0}
          />
        )}
        ...
        <div className="p-6">
          ...
          {step === "upload" && (
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Expected columns: Date, Merchant, Amount, Account,
                Category, Notes, Owner
              </p>
          ...
          {step === "map" && (
            <div>
              <p className="mb-4 text-sm text-muted-foreground">
                ...
                <strong>Amount</strong> (or Debit/Credit). Optionally map{" "}
                <strong>Account</strong>, <strong>Notes</strong>,{" "}
                <strong>Owner</strong>, etc.
              </p>
              ...
              <div className="mt-4 flex items-center justify-between">
                <p className="text-xs text-muted-foreground">
                  Showing {previewData.length} of {rawRows.length - 1}{" "}
                  rows
                </p>
          ...
          {step === "accounts" && (
            <div>
              <p className="mb-4 text-sm text-muted-foreground">
                {accountMappings.length} account
                {accountMappings.length !== 1 ? "s" : ""} found in your
                CSV. Assign a type to any new accounts.
              </p>
              {accountMappings.length === 0 ? (
                <div className="rounded-lg border border-border bg-muted/30 px-4 py-8 text-center text-sm text-muted-foreground">
                  No account column was mapped. All transactions will be
                  imported without an account.
                </div>
              ) : (
                        ...
                        >
                          ...
                          <td className="px-3 py-2">
                            <select
                              ...
                              className="rounded border border-border bg-background px-2 py-1 text-xs outline-none focus:border-accent disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              <option value="depository">
                                Depository
                              </option>
          ...
          {step === "categories" && (
            <div>
              <p className="mb-4 text-sm text-muted-foreground">
                {categoryMappings.length} unique categor{categoryMappings.length !== 1 ? "ies" : "y"} found
                in your CSV. Map each to an existing category or create new ones.
              </p>
              <div className="max-h-80 overflow-y-auto rounded-lg border border-border">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-muted/90 backdrop-blur">
                  ...
                  <tbody>
                    {categoryMappings.map((m, i) => (
                      <tr key={i} className="border-b border-border/50 last:border-0">
                        <td className="px-3 py-2 text-sm">{m.csvCategory}</td>
                        <td className="px-3 py-2">
                          {m.confidence === "exact" && (
                            <span className="inline-flex items-center gap-1 rounded-full bg-accent/15 px-2 py-0.5 text-[10px] font-medium text-accent">
                              <Check className="h-2.5 w-2.5" />
                          ...
                          {m.isNew ? (
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                next[i] = { ...m, mappedTo: e.target.value };
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (val === "__create_new__") {
                                  next[i] = { ...m, mappedTo: m.csvCategory, isNew: true, confidence: "none" };
                                } else {
                                  next[i] = { ...m, mappedTo: val, isNew: false };
                                }
                                setCategoryMappings(next);
                              }}
                              className="w-full rounded border border-border bg-background px-2 py-1 text-xs outline-none focus:border-accent"
              ...
              {categoryMappings.some((m) => m.isNew) && (
                <p className="mt-2 text-xs text-muted-foreground">
                  {categoryMappings.filter((m) => m.isNew).length} new categor{categoryMappings.filter((m) => m.isNew).length !== 1 ? "ies" : "y"} will be created.
                </p>
              )}
              <div className="mt-4 flex justify-end gap-2">
                <button
          ...
          {step === "owners" && (
            <div>
              <p className="mb-4 text-sm text-muted-foreground">
                Map CSV owner names to household members. Unmatched owners
                will default to you.
              </p>
              <div className="max-h-80 overflow-y-auto rounded-lg border border-border">
                <table className="w-full text-sm">
              ...
              <div className="mt-4 flex justify-end gap-2">
                <button
                  onClick={() => {
                    if (categoryMappings.length > 0) setStep("categories");
                    else setStep("accounts");
                  }}
                  className="inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground"
                >
                  <ArrowLeft className="h-3.5 w-3.5" />
          ...
          {step === "preview" && (
            <div>
              <p className="mb-4 text-sm text-muted-foreground">
                ...
                across {accountsPerGroup.length} account
                {accountsPerGroup.length !== 1 ? "s" : ""}.
                Duplicates will be automatically skipped.
              </p>

              {/* Per-account breakdown */}
              <div className="mb-4 grid gap-2">
                {accountsPerGroup.map(([name, count]) => (
                  <div
                    key={name}
                ...
                <div className="mt-2 flex flex-wrap justify-center gap-x-6 gap-y-1 text-sm">
                  ...
                  {newAccountCount > 0 && (
                    <span className="text-blue-400">
                      {newAccountCount} account
                      {newAccountCount !== 1 ? "s" : ""} created
                    </span>
                  )}
                </div>
              </div>
...
function StepIndicator({
  ...
  const steps: { key: Step; label: string }[] = [
    ...
    ...(hasCategories
      ? [{ key: "categories" as Step, label: "Categories" }]
      : []),
    ...(hasHousehold
      ? [{ key: "owners" as Step, label: "Owners" }]
      : []),
